...
class Background:
    ...
    def __init__(self, bgname, WIDTH, HEIGHT, WIN):
        self.bg_image = pygame.transform.scale(pygame.image.load(bgname).convert_alpha(), (WIDTH * 2,HEIGHT))
    ...
    def draw(self, WIN, player1, player2):
        #taking the center between players in order to keep the camera centered. The divide is because the camera movement is disorientingly fast if the value isn't cut down a bit
